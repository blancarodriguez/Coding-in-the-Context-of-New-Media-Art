// This example queries Twitter every 10 seconds for color, and blinks corresponding LEDs
...
// log into Twitter  
ConfigurationBuilder cb = new ConfigurationBuilder();
cb.setOAuthConsumerKey("qbJLuRfesbkQdCXsX8SBlNcLo");
cb.setOAuthConsumerSecret("lxFCwcSHou2zqM9Bkq3hpqscXes7MhW9jCb7jB0LeUKaiZawvP");
cb.setOAuthAccessToken("483265644-N4rgyJ6tP7qZGvS7xfuR4hxiiOcTJFcBXBKaVHGA");
cb.setOAuthAccessTokenSecret("RNlnnQNn2qQSqeYsR6IfgQkjTnVTBb1WRSaTaQqvvQ2U7");

// set your twitter object and your query (initialized above)
twitterInstance = new TwitterFactory(cb.build()).getInstance(); 
queryForTwitter = new Query("color"); 
...
// use the tweets to make a drawing!
  ...
  {
    if (word.toLowerCase().contains("red"))
    {
//every time you find the word "red", blink the red light
port.write('1');         //send a 8. This tells the red light to turn on. 
println("1");  
    }
  
    if (word.toLowerCase().contains("orange"))
    {
port.write('2'); 
println("2"); 
    }

    if (word.toLowerCase().contains("yellow"))
    {
port.write('3');
println("3");
    }
    
    if (word.toLowerCase().contains("green"))
    {
port.write('4'); 
println("4"); 
    }
    
    if (word.toLowerCase().contains("blue"))
    {
port.write('5'); 
println("5"); 
    } 
    
    if (word.toLowerCase().contains("white"))
    {
port.write('6'); 
println("6"); 
    }
...
{  
//wait 10 seconds-- this is not perfect because it feels slow
//but if you query too many times, you'll get booted off Twitter's rest API
delay (10000);
}